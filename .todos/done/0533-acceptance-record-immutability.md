...
- [x] `record_id` is a UUID or content-addressed value generated at run start, not derived from the output path
- [x] Duplicate `record_id` on insert raises a fatal error; `INSERT OR IGNORE` is gone from the acceptance write path
- [x] Output path is stored as a separate provenance field
- [x] Test confirms a duplicate ID attempt terminates with an error rather than silently succeeding
