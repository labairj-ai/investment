...
- Apply this check in both the section-3a FILLED path (0302 restart repair) and the section-3c PENDING_SUBMIT FILLED recovery path. The `BrokerOrder` returned by `get_order()` already carries `fill_qty`.
...
- [ ] Section-3a FILLED path appends `RECONCILIATION_UNAVAILABLE` when `sum(fill.qty) != broker_order.fill_qty`
- [ ] Section-3c PENDING_SUBMIT FILLED recovery performs the same check
- [ ] Section-3c PARTIALLY_FILLED recovery performs the same check
- [ ] Unit tests cover: correct qty passes, incomplete qty blocks, zero-fill (already blocked) unchanged
