...
- [ ] `validation_metrics` JSON includes `alpha_ci_low`, `alpha_ci_high`, `alpha_reliability` after training with sufficient CV folds
- [ ] `alpha_ci_low <= expected_alpha <= alpha_ci_high` always holds
- [ ] Reliability classification (`HIGH`/`MEDIUM`/`LOW`) present and config-driven
- [ ] Learning Lab model card displays uncertainty band and reliability label
- [ ] Models with 0 CV folds: CI fields are `null`, reliability is `"INSUFFICIENT_DATA"`
