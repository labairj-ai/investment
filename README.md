...
- **Portfolio AI** (`portfolio_ai.py`) — `phi-4-4bit` via MLX on Mac Studio; daily macro-aware insight card, per-holding macro risk scores, portfolio chat. Knows your actual CC positions, cost lots, holding ages, and behavioral patterns.
...
| **Per-Holding Macro Scores** | Each holding is scored 1–10 on four macro risk dimensions: rate sensitivity, inflation hedge, dollar sensitivity, geopolitical risk. Scores appear as colored dots in the holdings table (hover for breakdown). Generated by `phi-4-4bit` in batches of 8; refreshed weekly. |
...
- **MLX** (for portfolio AI) — optional; portfolio insight, macro scores, and portfolio chat require an MLX-compatible endpoint. Point `LLM_URL` in `.env` at an MLX server running `phi-4-4bit` (or any OpenAI-compatible API). On Apple Silicon: `mlx_lm.server --model microsoft/phi-4 --quantize 4bit --port 8080`. The server reads `LLM_URL` from the environment; if absent, portfolio AI features gracefully degrade to "AI unavailable".
...
**Per-Holding Macro Score Dots** — visible in the Holdings table next to each ticker. Four colored dots (green/yellow/red): rate sensitivity, inflation hedge, dollar sensitivity, geopolitical risk. Hover for the numeric score and one-line note. Scored by `phi-4-4bit` in batches of 8; refreshed weekly and after every nightly scheduler run.
...
├── portfolio_ai.py                  # Portfolio AI engine: daily insight, per-holding macro scores, portfolio chat system prompt; uses phi-4-4bit via MLX (LLM_URL); personal context from CC positions, cost lots, and sell transactions
