...
| **Test Suite & CI** | `tests/` directory with pytest (127 tests) covering: snapshot multi-lot aggregation and as_of fields; dependency checker (PRICE, THESIS_VERSION, POSITION_WEIGHT, MACRO_STATE, FINANCIAL_PERIOD, OPTION_IV, OPTION_EXPIRATION, OPTION_LIQUIDITY, ESTIMATE_REVISION, EVENT_CALENDAR, EARNINGS_DATE with authoritative DB supersession + heuristic fallback + unknown fail-safe, **CC_POSITION_STATE** — 0090); confidence scoring caps; outcome evaluator scenario math (HOLD, TRIM, TRIM+replacement, ALLOCATE, EXIT accepted with and without exec_rec, EXIT rejected, SELL_CC with exec_rec using real premium+strike vs payload, **multi-fill aggregation with weighted-average price and premium** — 0091); sell_trim action boundaries and SellStrength formula; **true valuation ratio percentile scoring** (`_RATIO_METRIC_MAP`, `_valuation_percentile`, `_v_score_from_percentile` — 0084); **portfolio_positions multi-lot aggregation** and weighted-average cost fix in generate_dashboard (0085); `agent_db` CRUD for executed_actions (position size fields, **fill_id idempotency**, **unique index on fill_id** — 0092), dep metadata JSON (0086), option quote snapshots (0087), earnings/events (0088), estimate history materiality gate (0089); **execution validation unit tests** (`tests/test_execute_validation.py` — 20 scenarios: date range, action-specific field validation for EXIT/TRIM/ALLOCATE and SELL_CC — 0092); and **lifecycle integration tests** (`tests/test_lifecycle.py` — 16 scenarios using in-memory DB: HOLD rejected, TRIM with execution_fraction, EXIT accepted with/without exec, EXIT rejected, dependency supersession, decision validation, full evaluate_matured_recommendations path, SELL_CC exec vs payload, **CC NO_ACTION hash changes on IV bucket change and open_cc flag change, guardian hash changes on max_weight_bucket and layer_drift_flag, unknown agent returns empty extras** — 0093). GitHub Actions workflow (`.github/workflows/test.yml`) runs the full suite on every push and PR against Python 3.12. All LLM calls are monkeypatched by an autouse fixture so tests never hit the network. |
...
│   ├── covered_call_agent.py        # CC Agent: reads per-thesis cc_policy (INCOME/UPSIDE_PRESERVATION/NONE) via _get_cc_policy(); NONE→skip; filters recs by max_preferred_delta and minimum_otm_pct; SELL_CC recs include full dep set (PRICE, OPTION_IV, OPTION_LIQUIDITY, OPTION_EXPIRATION, EARNINGS_DATE, THESIS_VERSION, CC_POSITION_STATE) with contract metadata (0090); writes top 3 option snapshots per analysis (0087); roll analysis via _analyze_roll() when open CC exists
│   ├── dependency_checker.py        # Dependency Checker: supersedes open recs when dependencies are stale; types: PRICE, THESIS_VERSION, POSITION_WEIGHT, MACRO_STATE, FINANCIAL_PERIOD, OPTION_IV, OPTION_EXPIRATION, OPTION_LIQUIDITY, ESTIMATE_REVISION, EVENT_CALENDAR, EARNINGS_DATE, CC_POSITION_STATE (0090); reads dependency_metadata_json for contract-specific context (0086)
