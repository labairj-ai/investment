...
| Feature | Description |
|---|---|
| **Portfolio AI Insight Card** | Daily macro-aware analysis generated by `Llama-3.3-70B-Instruct-4bit` (MLX). Reads your actual holdings, CC positions, cost lots, and behavior patterns. 9-field JSON: macro summary, portfolio alignment, risk flags, rebalancing take, tax timing, CC program note, tax opportunity, legislative watch, and key question of the week. If today's per-ticker news analysis has already run, its `_outlook` findings (top risk, top opportunity, tax watch, action items, per-ticker highlights) are injected into the insight prompt so the insight AI synthesizes both datasets rather than deriving from raw headlines alone. Refreshing kicks off async background generation; dashboard polls until ready. **Generation takes 3–5 minutes** (Llama-3.3-70B at ~5 tok/s; prompt is ~3,900 tokens, output capped at 1,200 tokens). Runs automatically at 7 AM so the result is cached by the time you open the dashboard. Collapsible accordion, defaults closed. |
...
- **MLX** (for portfolio AI) — optional; portfolio insight, macro scores, and portfolio chat require an MLX-compatible endpoint. Point `LLM_URL` in `.env` at an MLX server running `Llama-3.3-70B-Instruct-4bit` (or any OpenAI-compatible API). On Apple Silicon with ≥64 GB unified memory: `mlx_lm.server --model mlx-community/Llama-3.3-70B-Instruct-4bit --port 8080`. **Important:** if the model OOMs at default context, patch `config.json` in the model cache: set `"max_position_embeddings": 8192` (caps KV cache to ~2.5 GB vs the default 128K → ~41 GB). The server reads `LLM_URL` from the environment; if absent, portfolio AI features gracefully degrade to "AI unavailable".
...
| `POST /api/ai/chat` | SSE streaming portfolio chat. Body: `{messages: [{role, content}]}`. System prompt includes full holdings, layers, macro context, CC positions, cost lots, and behavior patterns. Streams `phi-4-4bit` tokens as `data: {"token":"..."}` events. Concurrency-guarded (429 if already streaming). |
...
**AI Insight Card** — collapsible accordion at the top of the dashboard grid, defaults closed. Powered by `Llama-3.3-70B-Instruct-4bit` via MLX. The prompt contains:
...
Scored one ticker at a time by `Llama-3.3-70B-Instruct-4bit` via MLX on Mac Studio. Runs automatically every **Saturday at 1 AM ET** via `investment-scores.timer` (systemd). History is stored in `holding_macro_scores_history`; sparklines appear as soon as 2+ runs have accumulated. Click the dots again to collapse.
...
├── ollama_client.py                 # Thin urllib wrapper for OpenAI-compatible APIs: generate() for JSON batch, stream_generate() for one-shot SSE, stream_chat() for multi-turn; reads LLM_URL from environment (supports both Ollama and MLX endpoints)
