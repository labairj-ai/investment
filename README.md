...
| Feature | Description |
|---|---|
| **Portfolio AI Insight Card** | Daily macro-aware analysis generated by `phi-4-4bit` (MLX). Reads your actual holdings, CC positions, cost lots, and behavior patterns — output references tickers by name, not by layer. 8-field JSON: macro summary, portfolio alignment, risk flags, rebalancing take, tax timing, CC program note, tax opportunity, and key question of the week. Refreshing kicks off async background generation; dashboard polls until ready. Collapsible accordion, defaults closed. |
| **Holdings News Card** | Per-ticker news headlines and AI summaries, displayed as a separate collapsible accordion card below the AI Insight card. Each ticker shows up to 6 headlines from WSJ, Barrons, MarketWatch, Yahoo Finance, CNBC, and Reuters; AI-generated per-ticker analysis includes news synthesis, rate sensitivity, trade/tariff exposure, and macro environment impact. Auto-refreshes at 6 AM, 12 PM, and 5 PM ET on weekdays. Defaults closed. |
| **Macro Context Bar** | Persistent top-of-dashboard bar showing live VIX, 10Y yield, 3M yield, 10Y–3M spread (with curve interpretation), CPI YoY, TLT move, GLD move, UUP (dollar), and today's top financial headlines. Data sourced from yfinance proxy instruments + FRED API; 30-minute cache. |
