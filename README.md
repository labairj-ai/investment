...
- **Ollama** (for AI Analysis) — optional; the rest of the dashboard works without it. Can run on the same server or on a separate always-on machine (e.g. a Mac Studio with Apple Silicon). The server reads `OLLAMA_URL` from the environment (default `http://127.0.0.1:11434`); set it in the systemd override to point to a remote host. Model needed:
  - `ollama pull llama3.3:70b` — all AI features: Buffett thesis, layer compare, nightly rankings, covered call analysis, chat (~40 GB; requires a machine with ≥48 GB unified memory)
...
├── ollama_client.py                 # Thin urllib wrapper for Ollama: generate() for JSON batch, stream_generate() for one-shot SSE, stream_chat() for multi-turn /api/chat (llama3.3:70b; timeout 180s)
