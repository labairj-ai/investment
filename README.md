...
| **Valuation-Aware Sell Scoring** | `_score_V()` in `sell_trim_agent.py` fetches 20 quarters of `price_to_earnings` history, ranks the current P/E at its historical percentile, and maps it to a 0–100 V score. Extreme threshold is overridable per-thesis via `valuation_framework.extreme_threshold` (stored in `investment_theses` on approval). Thesis intake generates a `valuation_framework` object: `primary_metric`, `secondary_metrics`, `attractive_threshold`, `fair_value_low/high`, `extreme_threshold`, `historical_period_years`, `growth_adjustment_method`. `_score_F()` computes YoY growth (same quarter prior year, ≥5 periods) or falls back to QoQ; TTM revenue is used when ≥8 periods are available. All comparisons prefer YoY over sequential-quarter to avoid seasonality noise. |
...
| **Dependency Checker** | Checks whether recommendation dependencies are still valid before surfacing a stale rec. Dependency types: `PRICE` (>5% move), `THESIS_VERSION` (version changed), `POSITION_WEIGHT` (>3pp drift), `MACRO_STATE` (score changed ≥20pts), `FINANCIAL_PERIOD` (new quarter filed), `OPTION_IV` (option expired or expiring ≤3 days), `EARNINGS_DATE` (earnings passed or within 7 days). `EVENT_CALENDAR`, `OPTION_LIQUIDITY`, and `ESTIMATE_REVISION` are registered stubs (log and pass until data sources are wired). Unknown types supersede the recommendation with a warning rather than silently passing. |
...
| **Test Suite & CI** | `tests/` directory with pytest covering: snapshot multi-lot aggregation and as_of fields, dependency checker (all 7 types + unknown fail-safe), confidence scoring caps, outcome evaluator scenario math (HOLD, TRIM, TRIM+replacement, ALLOCATE, EXIT accepted/rejected), sell_trim action boundaries and SellStrength formula, and `agent_db` CRUD for executed_actions and agent_runs audit fields. GitHub Actions workflow (`.github/workflows/test.yml`) runs the full suite on every push and PR against Python 3.12. All LLM calls are monkeypatched by an autouse fixture so tests never hit the network. |
...
| `POST /api/agents/recommendations/<id>/decision` | Body: `{decision, reason_code, notes}`. Writes to `user_decisions`, flips recommendation status to the decision value, and triggers the Preference Learner in a background thread. Returns updated recommendation. |
