...
| Feature | Description |
|---|---|
| **Portfolio AI Insight Card** | Daily macro-aware analysis generated by `phi-4-4bit` (MLX). Reads your actual holdings, CC positions, cost lots, and behavior patterns — output references tickers by name, not by layer. 8-field JSON: macro summary, portfolio alignment, risk flags, rebalancing take, tax timing, CC program note, tax opportunity, and key question of the week. Refreshing kicks off async background generation; dashboard polls until ready. |
...
**AI Insight Card** — auto-loads on page open; positioned at the top of the dashboard grid. Powered by `phi-4-4bit` via MLX. The prompt contains:
