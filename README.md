...
| **Outcome Evaluator** | Weekly background job (Sunday 2 AM ET) that scores matured recommendations (≥14 days old) against actual price history and SPY benchmark. Writes one row per (recommendation, horizon): equity horizons `1w`/`1m`/`3m`/`6m`/`12m`, CC horizons `at_expiry`/`30d_post`/`90d_post`. Four scenario columns per row: **A** `actual_return` (ticker hold), **B** `recommended_path_return` (EXIT→0, CC→premium±upside, else hold), **C** `hold_return` (always ticker hold), **D** `benchmark_return` (SPY return over same period). Horizons are only written after they have elapsed — no extrapolation. SPY daily prices are fetched via yfinance and cached in `spy_prices`. Alpha calculations: `AgentAlpha_vs_Hold = B−C`, `AgentAlpha_vs_SPY = B−D`, `UserOverrideAlpha = A−B`. Populates the Actual Return and Opp. Cost columns in the Decision Journal; aggregated alpha stats appear in the journal once ≥5 matured rows exist. |
| **Preference Learner** | Runs in the background after every user decision. Infers soft preferences from accepted/rejected decision history using exponential decay weighting (half-life 90 days) and sigmoid confidence (`100 × n/(n+10)`). Learned preferences: `action_acceptance_rate.{ACTION}` (per-action accept rate), `cc.preferred_delta` + `cc.preferred_dte` (from accepted SELL_CC payloads), `sell.score_threshold` (lowest score the user has accepted a TRIM/EXIT at). Minimum 5 decisions required before any preference influences display. Preferences are stored in `learned_preferences` and only affect the **Preference Fit** score on DQ cards — they never modify `recommendation_score`, `confidence`, or any evidence column. Hard rules in `strategy_config` are never modified. `GET /api/preferences` returns all learned preferences with confidence and sample_size. |
...
├── agent_db.py                      # SQLite CRUD layer for all agent tables; migrate() for schema versioning; key helpers: get_active_thesis(), update_pillar_status(), insert_recommendation() (auto-computes urgency_level), insert_finding(), list_runs(), get_run_full(), list_recommendations() (attaches preference_fit + trend_note per rec), close_recommendation(), get_lineage(), get_outcome_alpha_stats(), upsert_learned_preference(), get_learned_preferences(), _compute_preference_fit(), compute_urgency_level(), record_notification_event(), close_notification_event()
...
│   ├── outcome_evaluator.py         # Outcome Evaluator: runs Sunday 2 AM ET; scores matured recs (≥14 days) with 4 scenarios (A actual, B agent path, C hold, D SPY) across 5 equity horizons (1w/1m/3m/6m/12m) and 3 CC horizons (at_expiry/30d_post/90d_post); SPY prices cached in spy_prices via yfinance; alpha stats (B−C, B−D, A−B) aggregated in journal_summary when ≥5 rows exist
│   └── preference_learner.py        # Preference Learner: run_preference_learner() called in background thread after each user decision; infers action_acceptance_rate.{ACTION}, cc.preferred_delta, cc.preferred_dte, sell.score_threshold using exponential decay (half-life 90 days) and sigmoid confidence; writes to learned_preferences; never modifies strategy_config or any score column
└── out/
    ├── investment.db                # SQLite (PRODUCTION on optiplex — authoritative): price history, investment_theses, thesis_pillars, thesis_metrics, thesis_rules, cc_positions, cost_lots, sell_transactions, holding_macro_scores, holding_macro_scores_history, macro_score_summaries, agent_runs, agent_findings, recommendations (with supersedes_id + lineage_root_id for lineage, urgency_level column), critic_reviews, user_decisions, recommendation_outcomes (with horizon + 4 scenario columns), spy_prices, learned_preferences, preference_feedback, notification_events (urgency dispatch log + engagement tracking), candidate_universe, candidate_decisions, recommendation_dependencies, company_financials, company_estimates
