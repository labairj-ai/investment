...
| **Decision Queue** | Agent recommendation inbox in the **Decisions** tab. Displays all open `recommendations` rows prioritised by `0.35×Impact + 0.25×Urgency + 0.25×Confidence + 0.15×Synergy-Fit`. Each card shows ticker, action type (e.g. SELL_CC, REVIEW_THESIS), AI-generated rationale, critic verdict (approve / veto / flag), confidence score, **Preference Fit badge** (0–100, color-coded green/grey/red — visible once ≥5 decisions exist for that action type), escalating trend note (orange banner when severity has increased ≥3 consecutive assessments), **History** button (opens a lineage timeline showing all prior recommendations for that ticker + agent_type with dates, actions, confidence, and user decisions), and three decision buttons: **Accept**, **Reject**, **Defer**. A decision is recorded in `user_decisions`, closes the recommendation, and triggers the Preference Learner in the background. Counter on the Decisions nav button shows open count. |
...
├── agent_db.py                      # SQLite CRUD layer for all agent tables; migrate() for schema versioning; key helpers: get_active_thesis(), update_pillar_status(), insert_recommendation(), insert_finding(), list_runs(), get_run_full(), list_recommendations() (attaches preference_fit + trend_note per rec), close_recommendation(), get_lineage(), get_outcome_alpha_stats(), upsert_learned_preference(), get_learned_preferences(), _compute_preference_fit()
...
└── out/
    ├── investment.db                # SQLite (PRODUCTION on optiplex — authoritative): price history, investment_theses, thesis_pillars, thesis_metrics, thesis_rules, cc_positions, cost_lots, sell_transactions, holding_macro_scores, holding_macro_scores_history, macro_score_summaries, agent_runs, agent_findings, recommendations (with supersedes_id + lineage_root_id for lineage), critic_reviews, user_decisions, recommendation_outcomes (with horizon + 4 scenario columns), spy_prices, learned_preferences, candidate_universe, candidate_decisions, recommendation_dependencies, company_financials, company_estimates
