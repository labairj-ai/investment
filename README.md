...
| **Outcome Evaluator** | Weekly background job (Sunday 2 AM ET) that scores matured recommendations (≥14 days old) against actual price history and SPY benchmark. Writes one row per (recommendation, horizon): equity horizons `1w`/`1m`/`3m`/`6m`/`12m`, CC horizons `at_expiry`/`30d_post`/`90d_post`. Four scenario columns per row: **A** `actual_return` (what actually happened — 0.0 if user accepted an EXIT, `hold_return` if user rejected any action, estimated hold return otherwise), **B** `recommended_path_return` (EXIT→0, CC→`(min(S_T,K) − S_0 + premium) / S_0` capturing capped upside + premium, else hold return), **C** `hold_return` (always ticker hold), **D** `benchmark_return` (SPY return over same period). Three additional CC columns: `cc_strategy_return` (total CC path return), `cc_incremental_alpha` (CC strategy − hold), `actual_is_estimated` (flag for whether actual_return is observed or estimated). Horizons are only written after they have elapsed — no extrapolation. SPY daily prices fetched via yfinance and cached in `spy_prices`. Alpha calculations: `AgentAlpha_vs_Hold = B−C`, `AgentAlpha_vs_SPY = B−D`, `UserOverrideAlpha = A−B`. Populates the Actual Return and Opp. Cost columns in the Decision Journal; aggregated alpha stats appear in the journal once ≥5 matured rows exist. |
