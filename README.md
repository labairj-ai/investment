...
| **Test Suite & CI** | `tests/` directory with pytest (72 tests) covering: snapshot multi-lot aggregation and as_of fields; dependency checker (PRICE, THESIS_VERSION, POSITION_WEIGHT, MACRO_STATE, FINANCIAL_PERIOD, OPTION_IV, OPTION_EXPIRATION, OPTION_LIQUIDITY, ESTIMATE_REVISION, EVENT_CALENDAR, EARNINGS_DATE with authoritative DB supersession + heuristic fallback + unknown fail-safe); confidence scoring caps; outcome evaluator scenario math (HOLD, TRIM, TRIM+replacement, ALLOCATE, EXIT accepted with and without exec_rec, EXIT rejected, SELL_CC with exec_rec using real premium+strike vs payload); sell_trim action boundaries and SellStrength formula; `agent_db` CRUD for executed_actions (position size fields) and agent_runs (thesis_version + financial_period in input_snapshot); and **lifecycle integration tests** (`tests/test_lifecycle.py` — 11 scenarios using in-memory DB: HOLD rejected, TRIM with execution_fraction, EXIT accepted with/without exec, EXIT rejected, dependency supersession, decision validation, full evaluate_matured_recommendations path, SELL_CC exec vs payload). GitHub Actions workflow (`.github/workflows/test.yml`) runs the full suite on every push and PR against Python 3.12. All LLM calls are monkeypatched by an autouse fixture so tests never hit the network. |
...
│   ├── sell_trim_agent.py           # Sell/Trim Agent: SellStrength = 0.35V+0.25M+0.20T+0.20C; V-score is 5-factor composite (H=P/E absolute 0.30, G=PEG 0.25, FCF=sign/trend 0.20, E=analyst rec 0.15, C=price target 0.10) with re-normalisation for missing factors; TRIM/EXIT recs include POSITION_WEIGHT dependency
│   ├── covered_call_agent.py        # CC Agent: reads per-thesis cc_policy (INCOME/UPSIDE_PRESERVATION/NONE) via _get_cc_policy(); NONE→skip; filters recs by max_preferred_delta and minimum_otm_pct; SELL_CC recs include MACRO_STATE dependency; roll analysis via _analyze_roll() when open CC exists
│   ├── dependency_checker.py        # Dependency Checker: supersedes open recs when dependencies are stale; 5 types: PRICE, THESIS_VERSION, POSITION_WEIGHT (weight shift ≥2pp), MACRO_STATE (macro dimension shift ≥15), FINANCIAL_PERIOD (new financials filed)
